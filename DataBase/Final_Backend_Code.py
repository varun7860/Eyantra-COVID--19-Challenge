...
def read_excel(df):
    #df = pd.read_excel(path,names =['USERNAME', 'PASSWORD'])
...
def make_changes(data,dictionary,ID):
    ...
    if 'OXYGENERATOR' in dictionary.keys():
        ...
        curser.execute('''Update Personal set OXYGENERATOR = (?) where USERNAME = (?)''',(var,ID))
        
    if 'SANITISER' in dictionary.keys():
        var = dictionary.get('SANITISER')
        curser.execute('''Update Personal set SANITISER = (?) where USERNAME = (?)''',(var,ID))

    if 'MASKS' in dictionary.keys():
        var = dictionary.get('MASKS')
        curser.execute('''Update Personal set MASKS = (?) where USERNAME = (?)''',(var,ID))

    if 'VENTILATOR' in dictionary.keys():
        var = dictionary.get('VENTILATOR')
        curser.execute('''Update Personal set VENTILATOR = (?) where USERNAME = (?)''',(var,ID))

    if 'PPE' in dictionary.keys():
        var = dictionary.get('PPE')
        curser.execute('''Update Personal set PPE = (?) where USERNAME = (?)''',(var,ID))

    if 'VITAMINB12' in dictionary.keys():
        var = dictionary.get('VITAMINB12')
        curser.execute('''Update Personal set VITAMINB12 = (?) where USERNAME = (?)''',(var,ID))

    if 'VITAMINC' in dictionary.keys():
        var = dictionary.get('VITAMINC')
        curser.execute('''Update Personal set VITAMINC = (?) where USERNAME = (?)''',(var,ID))

    if 'SURGICALS' in dictionary.keys():
        var = dictionary.get('SURGICALS')
        curser.execute('''Update Personal set SURGICALS = (?) where USERNAME = (?)''',(var,ID))

    if 'BEDS' in dictionary.keys():
        var = dictionary.get('BEDS')
        curser.execute('''Update Personal set BEDS = (?) where USERNAME = (?)''',(var,ID))
...
def login_win():
    ...
    database = "Final.db"  #Put the name of your database here
